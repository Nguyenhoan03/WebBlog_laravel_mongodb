...
class HomeController extends Controller
{
    ...
    public function home()
    {
        ...
        foreach ($categorySlugs as $key => $slug) {
            ...
            $data[$key] = $category ? $this->getPostsByCategoryOrRecent($category->id, self::CATEGORY_POST_LIMIT) : collect();
        }
    
        $data['recent_posts'] = $this->getPostsByCategoryOrRecent(null, self::RECENT_POST_LIMIT);
    
        $data['most_popular'] = Post::select('id', 'title', 'content', 'image', 'slug', 'category_id', 'created_at', 'views', 'likes', 'comments_count')
            ->orderBy('views', 'desc')
            ->limit(3)
            ->get();
    
        $data['recommendations'] = Cache::remember('recommendations', 60, function () {
            return Post::select('author_id')
                ...
                ->get()
                ->map(function ($rec) {
                    $user = User::where('id', $rec->author_id)
